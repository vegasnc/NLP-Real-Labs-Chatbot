...
export default function ChatBox(props: PropsType) {
  ...
  const triggerPopup = () => {
    clearHistory();
    return <button className={styles.btnchatbox}><FontAwesomeIcon size='2x' icon={isOpenPopup ? faTimes : faComment} /></button>;
  }
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          question,
          history,
          isNewChain: history.length > 0 ? false : true
        }),
      });
      ...
      if (data.error) {
        ...
        if (!data.text.includes(SORRY_TEXT)) {
          data.sourceDocuments.map((obj: any, index: number) => {
            // if (isSuitableOne(data.text, obj?.pageContent)) {
            const propertyID = obj?.metadata?.source?.split("\\").pop().split('.').shift();
            const times = updateVisObjList(propertyID);

            // if same property is referenced more than 5 times, display the user information popup
            if (times > 2) {
              // reset referenced time
              resetVisObjProperty(propertyID);
              // display the user information popup for asking if they would like follow up
              props.toggle();
            }
            // }
          });
        ...
        <div className="mx-auto flex flex-col gap-4">
          <div className={styles.chatheader}>
            <h3 className="text-base font-bold leading-[1.1] tracking-tighter text-center">
              Find your perfect Reality Solution powered by Real Labs
            </h3>
          </div>
